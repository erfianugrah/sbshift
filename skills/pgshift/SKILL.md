---
name: pgshift
description: Drive the user's `pgshift` CLI — a typed Bun/TypeScript orchestrator for near-zero-downtime Postgres-to-Postgres migration via native logical replication (publication + slot + subscription → watch → reconcile → cutover → teardown). Use for cross-region Supabase moves, Supabase↔self-hosted, self-hosted↔self-hosted, same-region tier changes, or project splits — any PG15+ → PG15+ pair where a dump/restore downtime window is unacceptable. Covers the step pipeline and which steps no-op for non-Supabase, the IPv6 direct-vs-pooler trap (`SOURCE_DB_URL` pooler for admin/seed/reconcile + `SOURCE_REPLICATION_URL` direct host the target's walreceiver dials — verified live from a non-IPv6 box), what logical replication silently does NOT carry (DDL, roles, extensions, sequences, the Supabase `auth`/`storage` schemas) and the do-this-FIRST dump/restore pre-step, the `auth.users` cross-schema FK trap, sequence-resync-at-cutover collision guard, WAL-bloat + slot-lost + stuck-worker watchdog aborts, the cutover write-stop quiesce gate, chunked checksum reconciliation with per-bucket drill-down, the autonomous `run`/`status` CI entry points, and the three validation tiers (`test` unit / `test:integration` docker PG pair / `test:scale` 10M-row / `test:live` real throwaway Supabase pair). Sibling to `supabase` (the managed platform + auth/storage/config it wraps), `fly` (a place to run replication from with IPv6), and `infrastructure-stack`. Repo at `~/pgshift`; full runbook `docs/RUNBOOK.md`, design rationale `README.md`. Validated at 10M rows / 8.6 GB locally and end-to-end against a real cross-region Supabase pair (eu-central-1 → eu-west-1).
---
